...
export const changeInput =
  (This) => (key: string, value: any, root?: string) => {
    let data = {[key]: value};
    if (root) {
...
export const queryParams = (q: Object) =>
  Object.entries(q)
    .map(([k, v]) => `${k}=${v}`)
    .join('&');

export const maskDetail = (accNo: string) => {
  if (!accNo) {
    return accNo || '**** **** **** ****';
  }
...
export const limitLength = (string: string = '', maxLength: number) =>
...
export const formatCard = (pan: string) => {
  if (!pan || pan.length < 4) {
    return pan;
  }
...
export const getCardNumber = (pan: string) => pan.replace(/-/g, '');
...
export const mapCardsAndAccounts = (cards, accounts) => {
  accounts = accounts.map((a) => {
    ...
    cards = cards.map((c) => (a.nubanNumber === c.nuban ? {acc: a, ...c} : c));
    return updatedAcc;
  });
...
export const generateAuthorization = (
  ...
  let hashForAuth =
    'Basic ' + base64(`${deviceID}_${userID}` + ':' + authSecret);
